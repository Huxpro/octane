...
// answer "valid", so production compiles their bodies out and pays nothing per
// command. Development and the two-realm CI rig keep every
...
export function prepareLynxHostBatch<Node extends LynxElementRef>(
	...
	// check; production neither scans the batch nor allocates the set.
	const destroyedIds = LYNX_DEVELOPMENT ? new Set<number>() : null;
	...
	for (let index = 0; index < batch.commands.length; index++) {
		...
		if (command.op === 'create') {
			...
			if (typeof record.parent === 'number') {
				if (destroyedIds !== null && !destroyedIds.has(record.parent)) {
					throw hostError(
	...
	// first-tree adoption; a production steady-state commit never allocates or
	// populates it. Native-list accounting below remains a separate behavior cost.
	const finalIds = LYNX_DEVELOPMENT || firstTree !== undefined ? new Set<number>() : null;
	if (finalIds !== null) {
	...
	if (LYNX_DEVELOPMENT) {
		// The whole-tree validation pass only ever answers "valid" on a correct
		// build, so production compiles it out. List accounting remains below
		// because it contributes behavior. Time validation apart from staging so
		// the profile shows the share production sheds.
