	...
	it('validates the plan-aware wire: instantiate commands, announcements, and registry updates', () => {
		...
		expect(
			validateLynxBackgroundInboundMessage({
				...ready,
				plans: [
					{ id: '1a2b3c-9', canonical: 'h("view")' },
					{ id: 'z9y8x7-c', canonical: 't("row")' },
				],
			}),
		).toMatchObject({ type: 'main-ready', plans: [{ id: '1a2b3c-9' }, { id: 'z9y8x7-c' }] });
		...
		const registry = {
			...
			renderer: LYNX_TRANSPORT_RENDERER,
			type: 'plan-registry',
			plans: [{ id: '1a2b3c-9', canonical: 'h("view")' }],
		};
