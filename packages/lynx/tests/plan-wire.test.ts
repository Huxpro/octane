...
describe('lynx plan wire identity', () => {
	...
	it('declines identity for plans without a stable serialization', () => {
		...
		const withSymbolProp = universalPlan(LYNX_TRANSPORT_RENDERER, {
			kind: 'host',
			type: 'view',
			props: { [symbol]: true },
		});
		expect(lynxPlanWireId(withSymbolProp)).toBeNull();
		...
		for (const nested of [cyclic, new Date(0), accessor]) {
			const plan = universalPlan(LYNX_TRANSPORT_RENDERER, {
				kind: 'host',
				type: 'view',
				props: { nested },
			});
			expect(lynxPlanWireId(plan)).toBeNull();
		}
	});

	it('preserves static-value distinctions that the host can observe', () => {
		...
		expect(lynxPlanWireId(withOffset(-0))).not.toBe(lynxPlanWireId(withOffset(0)));
	});
});
...
describe('lynx plan registry', () => {
	it('registers eligible plans, resolves them, and notifies observers', () => {
		...
		const unsubscribe = onLynxPlanRegistered(({ id }) => observed.push(id));
		try {
			const id = registerLynxPlan(plan);
			expect(id).not.toBeNull();
			expect(observed).toEqual([id]);
			expect(resolveLynxPlan(id!)).toBe(plan);
			expect(lynxPlanRegistrySnapshot().map(({ id: planId }) => planId)).toContain(id);
	...
	it('registers nothing for plans the wire cannot instantiate', () => {
		const ineligible: UniversalPlanNode[] = [
			...
			{ kind: 'host', type: 'view', props: { bindtap: null } },
			{ kind: 'host', type: 'view', props: { 'main-thread:bindtap': null } },
		...
		for (const rootNode of ineligible) {
			...
			if (id !== null) {
				expect(lynxPlanRegistrySnapshot().map(({ id: planId }) => planId)).not.toContain(id);
			}
		}
	});
});
...
describe('lynx plan fold and expansion', () => {
	...
	it('keeps per-node commands for plans the main thread never announced', async () => {
		...
		expect('planInstances' in folded).toBe(false);
		// Matching a short ID is insufficient: the main-side canonical plan must
		// also match, so a hash collision across realms stays on the safe path.
		const collision = foldLynxPlanInstances(staged, (_planId, canonical) =>
			canonical.endsWith('different'),
		);
		expect(collision.commands).toEqual(staged.commands);
