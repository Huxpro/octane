...
import { PROGRAM_ADDRESSES_BUILD_INFO_KEY } from './program-addresses.js';
import {
...
export default function parallelOctaneLoader(source, sourceMap) {
	...
	this[COMPILER_OPTIONS_CONTEXT_KEY] =
		finalizer?.loaderItem?.data?.[COMPILER_OPTIONS_DATA_KEY] ?? undefined;
	...
	const withWorkerMetadata = (finish) => (error, output, outputMap, metadata) => {
		...
		finish(null, output, outputMap, {
			...
			__octaneParallelLoader: {
				...
				programAddresses: module.buildInfo?.[PROGRAM_ADDRESSES_BUILD_INFO_KEY] ?? null,
