...
export class OctaneRspackPlugin {
	...
	apply(compiler) {
		...
		// carrying a live function cannot cross into a worker. A backend module
		// reference is cloneable and loaded inside the worker; a caller that passes
		// the legacy live module still gets the serial loader. `structuredClone` is
		// asked rather than told, so any future non-cloneable option is covered.
