...
| `useState`, `useLinkedState`, `useReducer`, `useMemo`, `useRef`, `useCallback`, `useId`, `useEffectEvent`, `useEffect`, `useInsertionEffect`, `useLayoutEffect`, and `useSyncExternalStore` | Block selected | Hook cells publish only after host acceptance; memo/reducer identity, linked source generations, opaque IDs, and Effect Event bodies survive keyed moves. `useLinkedState` retains local edits while a changed source reconciles from the last accepted generation; rejected drafts do not advance it. Effect Event wrappers are inactive before their first ACK, keep calling the previous body while an update is in flight or rejected, switch to the new body only after ACK, and deactivate on scope disposal. A removed and later recreated row receives a fresh ID scope. The compact first screen evaluates both the lean linked-state pair and compiler-selected getter tuple, uses the same deterministic root-1 ID namespace as the complete main-thread renderer, and installs render-only Effect Event placeholders without running background bodies. Insertion, layout, and passive work use separate root-wide phases, so every insertion callback precedes every layout callback; rejected attempts publish none. Activity keeps insertion effects connected while hidden and disconnects layout/passive effects. Because Lynx is cross-thread, insertion runs after native acknowledgement and cannot synchronously block native paint. |
