...
| `useState`, `useReducer`, `useMemo`, `useRef`, `useCallback`, `useEffect`, `useInsertionEffect`, `useLayoutEffect`, and `useSyncExternalStore` | Block selected | Hook cells publish only after host acceptance; memo/reducer identity survives keyed moves. Insertion, layout, and passive work use separate root-wide phases, so every insertion callback precedes every layout callback; rejected attempts publish none. Activity keeps insertion effects connected while hidden and disconnects layout/passive effects. Because Lynx is cross-thread, insertion runs after native acknowledgement and cannot synchronously block native paint. |
