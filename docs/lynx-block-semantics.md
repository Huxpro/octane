...
| `useState`, `useLinkedState`, `useReducer`, `useMemo`, `useRef`, `useCallback`, `useId`, `useEffect`, `useInsertionEffect`, `useLayoutEffect`, and `useSyncExternalStore` | Block selected | Hook cells publish only after host acceptance; memo/reducer identity, linked source generations, and opaque IDs survive keyed moves. `useLinkedState` retains local edits while a changed source reconciles from the last accepted generation; rejected drafts do not advance it. A removed and later recreated row receives a fresh ID scope. The compact first screen evaluates both the lean linked-state pair and compiler-selected getter tuple, and uses the same deterministic root-1 ID namespace as the complete main-thread renderer, while the accepted background Block scope owns later updates. Insertion, layout, and passive work use separate root-wide phases, so every insertion callback precedes every layout callback; rejected attempts publish none. Activity keeps insertion effects connected while hidden and disconnects layout/passive effects. Because Lynx is cross-thread, insertion runs after native acknowledgement and cannot synchronously block native paint. |
