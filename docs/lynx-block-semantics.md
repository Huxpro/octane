...
| Immutable same-module component boundaries and component children | Block selected | The compiler proves the referenced binding is an immutable module-root function (including proven `memo` wrappers), lowers root/Provider child descriptors without an unaddressable shell plan, and the paired production build covers the boundary. Imported, dynamic, reassigned, or shadowed component bindings stay on Universal. |
| Render props | Block kernel proved | Sole expression children retain value semantics and transparent component chains preserve stateful descendants, but generic component-valued inputs remain outside production selection. |
