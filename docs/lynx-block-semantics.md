...
| `useState`, `useReducer`, `useMemo`, `useRef`, `useCallback`, `useEffect`, `useLayoutEffect`, and `useSyncExternalStore` | Block selected | Hook cells publish only after host acceptance; memo/reducer identity survives keyed moves, and layout/passive cleanup follows ACK, rejection, visibility, and deletion. |
