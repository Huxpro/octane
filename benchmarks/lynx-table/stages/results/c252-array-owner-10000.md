...
Measured at `e85c187be634a4a83f44dbb19a253eaef04ceb22`, on
Node v22.22.2 and 141.0.7390.37, 1-minute load 0.02 per CPU. The
...
|---:|---:|---:|---:|---:|---:|---:|
| 0 | 3.83 | 44.86 | 4.98 | 5.54 | 2741.3 | 525.5 |
| 1 | 3.81 | 44.86 | 4.98 | 5.54 | 2757.3 | 517.1 |
| 2 | 3.81 | 44.86 | 4.98 | 5.54 | 2680.0 | 525.3 |

Median retained over fresh: **1.17 MiB**
(1,229,904 bytes).
Median live over fresh: **41.05 MiB**.
...
|---|---:|---:|---:|---:|
| `native:system / JSArrayBufferData` | 14,745,600 | 14.06 | 1198.9% | 0 |
| `object:WeakRef` | 960,000 | 0.92 | 78.1% | 60,000 |
| `array:` | 519,032 | 0.49 | 42.2% | -25 |
| `hidden` | 37,891 | 0.04 | 3.1% | 254 |
...
| `native:PerformanceScriptTiming` | 560 | 0 | 0% | 5 |
...
| `native:NamedNodeMap` | 2,482,064 | 2.37 | 5.8% | 60,000 |
| `object:system / Context` | 2,359,980 | 2.25 | 5.5% | 99,999 |
| `string` | 2,113,712 | 2.02 | 4.9% | 39,954 |
| `object:Map` | 1,760,032 | 1.68 | 4.1% | 110,002 |
| `native:CSSStyleDeclaration` | 1,721,040 | 1.64 | 4% | 30,000 |
...
Second cycle: create 2701.6 ms, clear 590.6 ms; scalar moved
**0.56 MiB**
(582,588 bytes).

| bucket | bytes | MiB | share of retained | nodes |
|---|---:|---:|---:|---:|
| `native:system / JSArrayBufferData` | 15,204,352 | 14.5 | 2609.8% | 0 |
| `array:` | 524,432 | 0.5 | 90% | 3 |
| `code` | 48,748 | 0.05 | 8.4% | 269 |
| `hidden` | 14,185 | 0.01 | 2.4% | 17 |
| `native:PerformanceEventTiming` | 4,352 | 0 | 0.7% | 17 |
...
| `object shape:system / DescriptorArray` | 876 | 0 | 0.2% | 12 |
| `native:PerformanceLongAnimationFrameTiming` | 720 | 0 | 0.1% | 5 |
| `object shape:system / Map` | 720 | 0 | 0.1% | 18 |
| `native:PerformanceScriptTiming` | 560 | 0 | 0.1% | 5 |
| `native:<slot name="inline-truncation">` | 192 | 0 | 0% | 0 |
| `native:<slot part="slot">` | 192 | 0 | 0% | 0 |
| `object shape:system / PrototypeInfo` | 180 | 0 | 0% | 5 |
| `object:Object` | 148 | 0 | 0% | 6 |
| `object shape:(enum cache)` | 128 | 0 | 0% | 8 |
| `object shape:system / TransitionArray` | 104 | 0 | 0% | 2 |
...
| `number` | 24 | 0 | 0% | 2 |
| `object shape:system / Cell` | 24 | 0 | 0% | 3 |

Beyond the top 25: **0 MiB** across
0 further buckets, and it names no owner either.

## Retainers of `array:` — `afterClear`, median sample
...
`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`weak:27`-->
      `code` --`internal:wasm_weak_instance_list`-->
        `hidden` --`weak:0`-->
          `object:Instance` --`internal:exports`-->
            `object:Object` --`.__wbindgen_externrefs`-->
              `object:Table` --`[2]`-->
...
- `synthetic` via `internal:smi_string_cache`

A shortest chain from a GC root:

`synthetic` --`[1]`-->
  `synthetic` --`[22]`-->
    `synthetic` --`internal:smi_string_cache`-->

### Node 3 — 4,116 bytes

Held by 1 edge:

- `hidden` via `internal:slow_template_instantiations_cache`

A shortest chain from a GC root:

`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`internal:9 / V8PerContextData::context_`-->
      `hidden` --`internal:slow_template_instantiations_cache`-->
...
`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`internal:1 / V8PerContextData::context_`-->
      `hidden` --`internal:slow_template_instantiations_cache`-->
...
`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`internal:4 / V8PerContextData::context_`-->
      `hidden` --`internal:slow_template_instantiations_cache`-->
...
`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`internal:11 / V8PerContextData::context_`-->
      `hidden` --`internal:slow_template_instantiations_cache`-->
...
- `hidden` via `internal:fast_template_instantiations_cache`
...
`synthetic` --`[1]`-->
  `synthetic` --`[10]`-->
    `synthetic` --`internal:9 / V8PerContextData::context_`-->
      `hidden` --`internal:fast_template_instantiations_cache`-->
