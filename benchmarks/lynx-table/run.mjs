...
try {
	...
	for (const planId of mainLayer.lynxPlanRegistrySnapshot()) {
		workload.registerLynxPlan(mainLayer.resolveLynxPlan(planId));
	}
	...
	for (const rows of SCALES) {
		...
		// Create floor under plan instantiation (lynx-perf #5): the main thread
